        ...
        new(
            ...
            [
                new(
                    ...
                    ApiEndpoints:
                    [
                        new("GET", "/api/agents/review-queue?repo={owner}/{repo}&refresh=true&limit=10", "Programmatic endpoint for the same most-ready focus queue shown on the homepage review mode. The items field contains top Needs attention PRs; outsideNeedsAttentionItems contains open non-draft PRs filtered out of that top queue with bucket labels and exclusion reasons. Omit repo to use the dashboard-configured repositories; pass comma-separated repo values to constrain the queue."),
